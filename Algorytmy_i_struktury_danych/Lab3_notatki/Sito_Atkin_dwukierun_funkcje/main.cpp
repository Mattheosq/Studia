	...
	*/
	...
	for(int i=3; i<=grna_granica;i++) { // indeks 'i' odpowiada liczbie ktora sprawdzamy czy dla sito[i] wynosi true, jesli tak to 'i' jest liczba pierwsza , dla i=2 cur wskazywalby na 5, po czym go usunal, co wywoluje blad
		if(sito[i] == false){
			...
			if(cur->next != 0) // sprawdza czy za obecnym nodem, ktory usuwamy, znajduje sie jeszcze jakis node
				cur->next->prev = prev; // laczy nastepnik tego co usuwamy z jego poprzednikiem
...
void Zapisz(Node* head){
	...
	outfile.open("AtkinWy.txt"); // proba otworzenia pliku
	...
	if(outfile){  // sprawdza czy plik sie rzeczywiscie otworzyl, jezele plik wczesniej nie istnial, tworzy ten plik, jezeli plik juz istnial, nadpisuje go w calosci(nie dopisuje danych)
		...
		outfile.close();
		std::cout << "Wypisano dane do pliku AtkinWy.txt" << std::endl;
		getch();
